...
def print_operation_table(operation, num_rows, num_columns):
    result_table = []
    rows = [i+1 for i in range(num_rows)]
    colums = [i+1 for i in range(num_columns)]
    for i in rows:
        result_table.append([operation(i, j) for j in colums])
    for i in result_table:
        print(i)


print_operation_table(lambda a, b: a * b, 6, 6)
